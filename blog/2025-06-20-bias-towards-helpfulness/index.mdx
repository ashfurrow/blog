...
I've been using Cursor for coding tasks lately, trying to explore what kinds of work it performs well and poorly. It's pretty good at most simple tasks. Some complicated tasks can be done really smoothly, but then some simple tasks can stump it (usually in very niche domains). But then some tasks are _so big_ that they need to be broken into smaller pieces first – a classic project management problem.
...
My approach had been too much, too fast. To refine my approach, I tried using a conversational LLM instead of Cursor. I've generally not used conversational LLMs like ChatGPT until a few months ago, because I hadn't found them to be that useful and because it's rational to be skeptical of tech hype. But I had heard of developers using them to help think through ideas and project plans, so why not try? 
