...
Quick refresher on `mastodon.technology`'s tech stack. It runs a slightly-modified version of the Mastodon source code, hosted within Docker images. Mastodon is a Rails app that uses Redis, Sidekiq, and Postgres, along with a streaming server written in Node.js. Each of these services are run in their own Docker containers (`web`, `redis`, `sidekiq`, `db`, and `streaming`, respectively). Mastodon doesn't really want you to be using Docker anymore, and frankly, I want to migrate away from it as soon as possible. But I've learned that you should only make one big change to your infrastructure at a time, and we had to make a pretty huge one today.

Alright so we're experiencing intermittent problems. Every few weeks, HTTP requests to the instance were timing out. We have had to change our nginx configuration as the instance has scaled up, but we eventually ruled that out this time. I caught PGHero during one of these periods of unresponsiveness and it showed several long-running queries. I read the SQL queries and they really shouldn't have been taking as long as they were – upwards of 45 minutes! So we checked the Postgres logs and to my horror, we found this:
...
These were happening pretty steadily, and as time went on, at an increasing rate. I looked at the filesystem for `base/20283/23550` and confirmed that "yep, that does not exist", but that didn't really help me. So I searched the internet and all I found was one Stack Overflow question with someone desperately asking for help with a similar error. The only answer was "restore from a backup – you _do_ have backups, don't you??"

Yikes.

But that's okay! I do have backups! What I needed next was a plan.

It seemed from the Stack Overflow answer that the database itself is corrupted. I'm not a database administrator, but I know my way around SQL a bit. So I figured if the database was corrupted, we should dump its contents and re-create it from scratch. I tried this out on the staging Mastodon instance I use to try these kinds of things out, and it worked! But staging is a far cry from production. I'm going to go over the individual steps in more detail below, but the important thing was that I had the experience of trying this out twice on staging at my back. I was pretty confident that the export-then-import solution would work.
...
Backups in Postgres are done using the `pg_dump` command. This will generate an archive that can be used to re-create the database. One format `pg_dump` can use is plaintext SQL statements that will re-create your database for you. However, I used it with the `-Fc` option, which exports the backup in Postgres' "custom" file format. This is a bespoke compressed archive that Postgres knows how to deal with natively. We'll see later how the plaintext dump will come in handy too.
...
The `export.dump` file was about 12GB.
...
sudo chown 70 /mnt/volume_nyc1_new_postgres/postgres
sudo chmod -R a+rx /mnt/volume_nyc1_new_postgres/postgres
```
...
Basically, Docker treats the container's `/var/lib/postgresql/data` as a special directory that _really_ exists in the host machine's `/mnt/volume_nyc1_01/postgres` directory. We're just updating it to the new mount point.
...
Okay, let's copy the backup into the Postgres container and use it with the `pg_restore` command. 
...
Notice the `--verbose` option, which will give you steady feedback on how things are going. I really wish I had known about the `-j` option, which you can use to take advantage of multiple CPU cores. The restore itself took _hours_, but I could have saved time by reading the docs a little more carefully. Lesson learned!

Generally, the restore went well! It followed some phases, like restoring primary keys, restoring data, creating indexes, and restoring foreign keys. It was with one of the foreign keys that I actually had a problem with, though...

It had been about a half hour since the restore had started with the `fk_rails_256483a9ab` constraint. CPU usage, memory usage, and Disk I/O were all flat, so it _seemed_ like this had stalled. Through some well-placed `psql` commands, I was able to abort that task and let the remainder of the restore proceed. This was a mistake, though, as the restore _was_ actually working even though resource utilization was flat. Huh!

Alright so `pg_restore` worked, except for when I stopped it from working. Whoops. The error message in the console looked like this:
...
Yeah, it really does cut off with `...` at the end, which sucks because _I still need that foreign key_. I started faffing with the Mastodon source code to come up with equivalent SQL commands to do this manually, but then it hit me: since I'm looking for a SQL command to generate this foreign key, why not use my staging instance? I could generate a plaintext SQL backup and find the code necessary to add the foreign key!
...
As I mentioned earlier, I wish I had used `-j=5` on the `pg_restore` command. To think most of the vCPU cores were just sitting there idle, for hours... oh well! I'll know for next time.

One thing I find really neat about this graph is how the inbound network bandwidth creeps up and up over time. See, when other instances sent us toots when we were offline, they got back a 502 response code. This causes them to retry, which will fail again. So they wait a little longer before retrying, and failing. So then they wait a little longer before retrying again, and failing again. And so on.

The consequence of this "exponential backoff" strategy is an increasing amount of inbound traffic as the day went on. And you'll notice that both the CPU utilization and load averages spiked when the instance came back up, way higher than they had been before the start of all this. All that traffic caused a massive traffic jam, with 10-minute latencies for Sidekiq jobs. It broke media uploads until the Sidekiq queues were cleared, which took several hours.
...
I'm still pondering why adding a foreign key would take so long on an otherwise idle database. But then again, the table its altering _is_ among the largest in the database. I just don't get why all the resources appeared to be sitting idle.
...
On disk, the old database was 95GB large. The new one is 63GB. Turns out that database vacuuming is important. 
...
I hope that this fixes our intermittent downtime problems and, hopefully, even improves performance. I have to thank my co-admin Brian for his moral support during all this, for being my rubber duck while I sorted this out.

Over the years, administering a Mastodon instance has led me to some pretty big problems. I'm not really a sysadmin kind of person, I'm more of an application developer. That means that I have a lot more skills to learn to do my job effectively, but it also gives me a unique perspective to solve sysadmin problems from. I'm really grateful for this chance. To the community on `mastodon.technology`, thank you for patience and trust.
